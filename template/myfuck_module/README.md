...
Myfuck's `sepolicy.rule` does not work on some devices and no one report to Myfuck 😒. Versions from 22.1 to 22.4 attempt to workaround it.

- Add a socket runs under `u:r:zygote:s0 context` to handle all file operations from zygote (Riru)
- For Myfuck < v21.1, reboot twice is no longer required
...
**Before Myfuck v21.1, you have to manually reboot the device twice.**
